...
class CASBackendTests(TestCase):
    ...
    def setUp(self):
        ...
        self.inst = Institution.objects.create(
            name="Test Inst",
            slug="test-inst",
            cas_server_url="https://fed.testinst.edu/",
        )
        self.inst2 = Institution.objects.create(
            name="Other Inst",
            slug="other-inst",
            cas_server_url="https://fed.other.edu/",
        )
    ...
    def test_cas_backend_create_user(self, mock_verify_ticket):
        ...
        self.assertTrue(
            User.objects.filter(username="cas-test-inst-newuser").exists()
        )
        self.assertEqual(
    ...
    def test_cas_backend_existing_user(self, mock_verify_ticket):
        ...
        user1 = User.objects.create(
            username="johndoe@gmail.com", email="john.doe@aol.com"
        )
        ...
        InstitutionAccount.objects.create(
            profile=fakeout.uniauth_profile,
...
class EmailBackendTests(TestCase):
    ...
    def setUp(self):
        ...
        self.mary = User.objects.create_user(
            username="marysue@outlook.com",
            email="mary.sue@gmail.com",
            password="marypass",
        )
        LinkedEmail.objects.create(
            profile=self.mary.uniauth_profile,
            address="alternate@gmail.com",
            is_verified=True,
        )
        LinkedEmail.objects.create(
            profile=self.mary.uniauth_profile,
            address="pending@gmail.com",
            is_verified=False,
        )
        self.cas = User.objects.create_user(username="cas-inst-netid123")
        self.tmp = User.objects.create_user(
            username="tmp-0123_456", password="tmppass"
        )
        self.perm = User.objects.create_user(
            username="permid555", password="permpass"
        )
        self.mr = User.objects.create_user(
            username="mrjones@gmail.com",
            email="mrjones@gmail.com",
            password="mrpass",
        )
        self.ms = User.objects.create_user(
            username="msjones@gmail.com",
            email="msjones@gmail.com",
            password="mspass",
        )
        LinkedEmail.objects.create(
            profile=self.mr.uniauth_profile,
            address="sharedjones@gmail.com",
            is_verified=True,
        )
        LinkedEmail.objects.create(
            profile=self.ms.uniauth_profile,
            address="sharedjones@gmail.com",
            is_verified=True,
        )
...
class LinkedEmailBackendTests(EmailBackendTests):
    ...
    def test_linked_email_backend_valid_credentials(self):
        ...
        user = backend.authenticate(
            None, email="johndoe@gmail.com", password="johnpass"
        )
        self.assertEqual(user, self.john)

        # Log in with email (under "username" parameter) + password
        user = backend.authenticate(
            None, username="mary.sue@gmail.com", password="marypass"
        )
        self.assertEqual(user, self.mary)

        # Log in with verified linked email
        user = backend.authenticate(
            None, email="alternate@gmail.com", password="marypass"
        )
        self.assertEqual(user, self.mary)

        # Log in with shared email address
        user = backend.authenticate(
            None, email="sharedjones@gmail.com", password="mspass"
        )
        self.assertEqual(user, self.ms)
        user = backend.authenticate(
            None, email="sharedjones@gmail.com", password="mrpass"
        )
    ...
    def test_linked_email_backend_invalid_credentials(self):
        ...
        user = backend.authenticate(None, username="dne", password="johnpass")
        self.assertEqual(user, None)
        user = backend.authenticate(
            None, email="dne@gmail.com", password="marypass"
        )
        self.assertEqual(user, None)
        ...
        user = backend.authenticate(
            None, email="johndoe@gmail.com", password=None
        )
        self.assertEqual(user, None)
        user = backend.authenticate(
            None, email="johndoe@gmail.com", password=""
        )
        self.assertEqual(user, None)
        user = backend.authenticate(
            None, email="johndoe@gmail.com", password="marypass"
        )
        self.assertEqual(user, None)

        # Can't log in with username
        user = backend.authenticate(
            None, email="marysue@outlook.com", password="marypass"
        )
        self.assertEqual(user, None)
        user = backend.authenticate(
            None, username="permid555", password="permpass"
        )
        self.assertEqual(user, None)

        # Can't log in with unverified email
        user = backend.authenticate(
            None, email="pending@gmail.com", password="marypass"
        )
        self.assertEqual(user, None)

        # Can't log in as a temporary user
        user = backend.authenticate(
            None, username="tmp-0123_456", password="tmppass"
        )
        self.assertEqual(user, None)
